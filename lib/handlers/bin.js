...
module.exports = Observable.extend({
  ...
  applyProcessors: function (bin) {
    if (bin.settings && bin.settings.processors) {
      console.log('processor being applied');
      return new RSVP.all(Object.keys(bin.settings.processors).map(function (panel) {
        var processorName = bin.settings.processors[panel],
            processor = processors[processorName],
            code = bin[panel];
        console.log('does processor exist for ' + panel);
        if (processor) {
          console.log('yes');
          bin['original_' + panel] = code;
          return processors.run(processorName, code).then(function (output) {
    ...
    return new RSVP.Promise(function (resolve, reject) {
      ...
      if (bin.settings && bin.settings.processors) {
        for (var panel in bin.settings.processors) { // jshint ignore:line
          var processorName = bin.settings.processors[panel],
              processor = processors[processorName],
              code = bin[panel];
          if (processor) {
            bin['original_' + panel] = code;
            processor(code).then(function (output) {
              bin[panel] = output;
              resolve(bin);
            }, reject);
          } else {
    ...
    this.applyProcessors(bin).then(function (bin) {
      ...
      if (formatted) {
        if (options.analytics !== false) {
          helpers.analytics(true, function (err, analytics) {
      ...
      fn(error || 'Could not render output');
    });
